...
int test_front() {
    if (direction == 0) {
        return path_poundary[j - 1][i];
    }
    if (direction == 1) {
        return path_poundary[j][i + 1];
    }
    if (direction == 2) {
        return path_poundary[j + 1][i];
    }
...
void loop() {
    ...
    if (calc_distance() < 25) {
        ...
        if (front_open() == 0 || front_open() == -1) {
            forward();

        }
        else if (right_open() == true) {
            right();
        }
        else if (left_open() == true) {
            left();
